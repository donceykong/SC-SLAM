	...
	const int& loop_key1)
{
	...
	{
		RCLCPP_WARN(node_->get_logger(), "keyFrameCloud too little points 1");
		return;
	}
	...
	if(pub_scan_of_scan2map->get_subscription_count() != 0)
	{
		...
		scan_cloud_msg.header.stamp = node_->now();
	...
	if(pub_map_of_scan2map->get_subscription_count() != 0)
	{
		...
		map_cloud_msg.header.stamp = node_->now();
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "\033[1;34m[IntraLoop<%d>] [%d]-[%d] ICP failed (%.2f > %.2f). Reject.\033[0m",
	...
	RCLCPP_DEBUG(node_->get_logger(), "\033[1;34m[IntraLoop<%d>] [%d]-[%d] ICP passed (%.2f < %.2f). Add.\033[0m",
		id_, loop_key0, loop_key1, fitness_score, fitness_score_threshold_);
...
void distributedMapping::performExternLoopClosure()
{
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "\033[1;33m[LoopClosure] Loop has added. Skip.\033[0m");
		return;
	}
	...
	*scan_cloud_ds = *transformPointCloud(*scan_cloud_ds, loop_pose0);
	...
	{
		RCLCPP_WARN(node_->get_logger(), "keyFrameCloud too little points 2");
		return;
	}
	if (!scan_cloud_ds->is_dense || !map_cloud_ds->is_dense)
	{
		RCLCPP_WARN(node_->get_logger(), "keyFrameCloud is not dense");
		return;
	}
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "\033[1;35m[InterLoop<%d>] [%d][%d]-[%d][%d] RANSAC failed (%.2f < %.2f). Reject.\033[0m",
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "\033[1;35m[InterLoop<%d>] [%d][%d]-[%d][%d] ICP failed (%.2f > %.2f). Reject.\033[0m",
			id_, inter_loop.robot0, inter_loop.index0, inter_loop.robot1, inter_loop.index1,
	...
	RCLCPP_DEBUG(node_->get_logger(), "\033[1;35m[InterLoop<%d>] [%d][%d]-[%d][%d] inlier (%.2f < %.2f) fitness (%.2f < %.2f). Add.\033[0m",
		id_, inter_loop.robot0, inter_loop.index0, inter_loop.robot1, inter_loop.index1,
		new_correspondences.size()*1.0/correspondences->size()*1.0, ransac_threshold_,
		inter_loop.noise, fitness_score_threshold_*2);
