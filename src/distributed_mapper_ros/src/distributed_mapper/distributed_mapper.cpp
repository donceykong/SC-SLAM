...
DistributedMapper::createLinearOrientationGraph(){
  ...
  // Use GTSAM 4.2's built-in version which handles the anchor correctly for the solver,
  // then optionally replace noise models with between-factor noise
  rot_subgraph_ = InitializePose3::buildLinearOrientationGraph(pose3_graph);

  if(use_between_noise_){
    // Rebuild with custom between noise, using GTSAM's anchor approach
    GaussianFactorGraph custom_graph;
    // Copy the anchor prior from GTSAM's built-in graph (last factor)
    for(size_t i = 0; i < rot_subgraph_.size(); i++){
      auto factor = rot_subgraph_.at(i);
      if(factor->keys().size() == 1){
        // This is the anchor prior - keep it as-is
        custom_graph.push_back(factor);
      }
    }
    // Rebuild binary factors with custom between noise
    for (const boost::shared_ptr<NonlinearFactor> &factor: pose3_graph) {
      boost::shared_ptr<BetweenFactor<Pose3> > pose3_between =
          boost::dynamic_pointer_cast<BetweenFactor<Pose3> >(factor);
      if (pose3_between){
        Matrix3 Rij = pose3_between->measured().rotation().matrix();
        SharedDiagonal model = evaluation_utils::convertToDiagonalNoise(pose3_between->noiseModel());
        ...
        custom_graph.add(key1, -I9, key2, M9, zero9, model);
      }
    }
    rot_subgraph_ = custom_graph;
  }
}
