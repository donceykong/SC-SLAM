	...
	int id)
{
	...
	robots[id].time_cloud_input = rclcpp::Time(msg->header.stamp).seconds();
	...
	int id)
{
	...
	for(int i = 0; i < msg->pose_id.size(); i++)
	{
		...
		{
			RCLCPP_DEBUG(node_->get_logger(), "Stop optimization<%d>. Key %d %d doesn't exist.", id_, symbol.chr(), symbol.index());
	...
	RCLCPP_DEBUG(node_->get_logger(), "neighborRotationHandler<%d> from robot %d done? %d (%d/%d]", id_, id, msg->estimation_done,
		(int)optimizer->getNeighboringRobotsInit().size(), (int)(optimization_order.size() - 1));
	...
	if(optimizer->getNeighboringRobotsInit().size() == optimization_order.size() - 1)
	{
		if(!estimation_done)
		{
			...
			{
				RCLCPP_WARN(node_->get_logger(), "Stopping rotation optimization<%d> : %s.", id_, ex.what());
			...
			RCLCPP_DEBUG(node_->get_logger(), "--->Rotation estimation<%d> iter:[%d/%d] change:%.4f.", id_,
	...
	int id)
{
	...
	RCLCPP_DEBUG(node_->get_logger(), "neighborPoseHandler<%d> from robot %d, done? %d [%d/%d]", id_, id, msg->estimation_done,
		(int)optimizer->getNeighboringRobotsInit().size(), (int)(optimization_order.size() - 1));
	...
	if(optimizer->getNeighboringRobotsInit().size() == optimization_order.size() - 1)
	{
		if(!estimation_done)
		{
			...
			{
				RCLCPP_WARN(node_->get_logger(), "Stopping pose optimization<%d> : %s.",id_,ex.what());
				abortOptimization(true);
			}
			...
			RCLCPP_DEBUG(node_->get_logger(), "--->Pose estimation<%d> iter:[%d/%d] change:%.4f.", id_,
...
void distributedMapping::makeDescriptors()
{
	...
	if(cloud_for_decript_ds->empty())
	{
		LOG(WARNING) << "makeDescriptors[" << id_ << "]: empty cloud after downsampling, skipping." << endl;
		return;
	}
	...
	const rclcpp::Time& timestamp)
{
	...
	auto quat = pose_between.rotation().toQuaternion();

	geometry_msgs::msg::TransformStamped t;
	t.header.stamp = timestamp;
	t.header.frame_id = world_frame_;
	t.child_frame_id = robots[id_].odom_frame_;
	t.transform.translation.x = pose_between.translation().x();
	t.transform.translation.y = pose_between.translation().y();
	t.transform.translation.z = pose_between.translation().z();
...
void distributedMapping::updateOptimizer()
{
	...
	RCLCPP_DEBUG(node_->get_logger(), "priorOwner<%d> %d",id_, prior_owner);
...
void distributedMapping::outliersFiltering()
{
	...
	if(use_pcm_)
	{
		...
		for(const auto& neighbor : neighbors_within_communication_range)
		{
			...
			RCLCPP_DEBUG(node_->get_logger(), "outliersFiltering<%d>, other=%d, max clique size=%d, removed=%d", id_, neighbor,
				(int)max_clique.size(), max_clique_info.second);
		}
	}
}
...
void distributedMapping::removeInactiveNeighbors()
{
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "Stop optimization<%d>, there are inactive neighbors.",id_);
		abortOptimization(false);
	}
}
...
void distributedMapping::failSafeCheck()
{
	...
	{
		RCLCPP_DEBUG(node_->get_logger(), "No progress<%d>, Stop optimization", id_);
		abortOptimization(false);
	}
}
...
void distributedMapping::incrementalInitialGuessUpdate()
{
	...
	if(pub_keypose_cloud->get_subscription_count() != 0)
	{
		...
		keypose_cloud_msg.header.stamp = node_->now();
	...
	const OptimizerState& state)
{
	switch(state)
	{
		case OptimizerState::Idle:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>Idle", id_);
			break;
		case OptimizerState::Start:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>Start", id_);
			break;
		case OptimizerState::Initialization:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>Initialization", id_);
			break;
		case OptimizerState::RotationEstimation:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>RotationEstimation", id_);
			break;
		case OptimizerState::PoseEstimationInitialization:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>PoseEstimationInitialization", id_);
			break;
		case OptimizerState::PoseEstimation:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>PoseEstimation", id_);
			break;
		case OptimizerState::End:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>End", id_);
			break;
		case OptimizerState::PostEndingCommunicationDelay:
			RCLCPP_DEBUG(node_->get_logger(), "<%d>PostEndingCommunicationDelay", id_);
			break;
	}
