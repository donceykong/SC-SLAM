...
// QoS profiles matching ROS1 TCP behavior (reliable, no drops)
inline rclcpp::QoS qos_reliable(int depth = 10) {
    return rclcpp::QoS(rclcpp::KeepLast(depth)).reliable().durability_volatile();
}
inline rclcpp::QoS qos_imu() {
    return rclcpp::QoS(rclcpp::KeepLast(2000)).reliable().durability_volatile();
}
inline rclcpp::QoS qos_lidar() {
...
public:
    ...
    // Initial pose offset (for testing distributed loop closure)
    ...
    ParamServer(const std::string& node_name) : rclcpp::Node(node_name)
    {
        ...
        initialYawOffset = this->declare_parameter<float>("lio_sam.initialYawOffset", 0.0f);
    }
